...
public:
  ...
  void remove_extra_subtemplates()
  {
    for (int s = subtemplate_count-1; s > 1; --s) {
      if (count_needed[s]) {
        for (int i = s - 1; i > 0; --i) {
          if (count_needed[i]) {
            if (check_rooted_isomorphism(subtemplates[s], subtemplates[i]))
            {
              if (active_children[parents[i]] == i)
              {
                active_children[parents[i]] = s;
                count_needed[i] = false;
              }
              else if (passive_children[parents[i]] == i)
              {
                passive_children[parents[i]] = s;
                count_needed[i] = false;
              }            
            }
          }
