...
int get_part(int id, int* part_offsets, int num_parts)
{
	for (int i = 0; i < num_parts; ++i)
		if (id >= part_offsets[i] && id < part_offsets[i+1])
			return i;

	printf("error bad vert id %d\n", id);
	exit(0);
	return -1;
}
